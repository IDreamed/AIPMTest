...
- 公共课：语文、数学、英语。
- 专业课按 19 个专业大类筛选。
- 文化课科目和专业课大类都按后台动态配置理解；前端筛选统一为一级“文化课/专业课”，二级用同一套“资源概览卡片”交互。
...
- 试卷结构按真实试卷构建：第一大题、第二大题、第三大题等；每个大题对应一种题型。
- 综合题为一个主题干 + 多个子题；综合题子题仍是单选、多选、判断、填空、简答等常规题型。
- 完成练习后系统判卷客观题并给出解析。
- 试卷刷题允许中途退出，按钮文案为“保存退出”，并支持继续刷题。
- 标记本题只属于本次试卷记录；题号导航和解析页展示标记状态，不进入学习中心。
- 题号状态标准：未答题、已答题、正确、错误、已评分；标记为叠加状态。
- 试卷解析页 `#/paper-analysis` 展示答题总结、题号导航和单题解析；解析内容来自后台富文本，可能包含文字、图片、视频。
...
- 学生状态主要由“权限 + 考试状态 + 是否交卷”推导，不优先依赖 mock 覆盖字段。
- 考试中心筛选包含：全部考试/我的考试、考试类型、文化课/专业课、科目/专业大类、考试状态。
- “我的考试”用于和学习中心区分：注册用户展示自己的公开考试，班级学生展示公开考试和当前班级授权学校联考，游客提示登录。
- 考试中心不做“学生考试状态”相关的筛选。
- 考试中心列表展示每个考试的“考试状态”和“学生考试状态”。
- 考试列表展示考试时间，包含开始时间和结束时间。

### 考试详情

- 展示单场考试基础信息、考试介绍、状态、权限、考试时间安排、成绩/排行摘要。
- 考试介绍是后台富文本配置，学生端详情页靠上展示，可包含文字、图片、视频或资料链接。
- 考试时间安排展示开始时间、结束时间、成绩公示时间。
- 考试详情不展示题目作答区。
- 考试详情不展开完整排名榜单，完整排行进入 `#/exam-rank`。
- 结果展示方式由后台配置结果决定：
  - `rankEnabled = true`：成绩 + 排名。
  - `rankEnabled = false`：仅成绩。
- 学生端不提供考试相关的设置。
- 成绩区使用“得分/总分、正确率、答题用时、个人排名”等结果数据，不拆主观题/客观题。
- `评审中 + 已交卷` 展示“已交卷，成绩评审中”，提示成绩公示前不可查看成绩、排行和题目解析。
...
- 交卷才算参加考试。
- 不展示保存并下一题。
- 考试答题页适配试卷答题页结构：大题分组、综合题主题干 + 多子题、题号导航、标记本题。
- 考试交卷前使用确认弹窗。
- 考试成绩与解析页 `#/exam-analysis` 展示得分/总分、题目状态、单题解析和综合题解析。
- 考试排行页 `#/exam-rank` 独立展示考生排行、学校排行，并包含分页与每页 20/30/50 条配置。
...
- 首页顶部平台介绍已改为更偏宣传性文案。
- 首页推荐课程已从轮播改为 4 列 x 2 行平铺卡片，使用封面占位图 + 课程信息样式；推荐课程 mock 数据补足 8 条。
- 资讯中心详情弹窗已改为模拟富文本展示。
- 推荐课程试看页已去掉重复目录 Tab。
- 试卷中心筛选已统一为“文化课/专业课”一级筛选，二级文化课科目和专业大类均使用资源概览卡片交互，去掉重复下拉。
- 试卷中心下级页面已补齐：
  - `#/paper-answer`：试卷答题页，支持保存退出、继续答题、标记本题、真实大题结构和综合题子题。
  - `#/paper-analysis`：试卷解析页，展示答题总结、题目状态、单题解析和富文本解析占位。
- 题目状态标准已统一：未答题、已答题、正确、错误、已评分；标记为叠加状态。
- 考试中心已完成主流程：
  - 列表支持全部考试/我的考试、公开考试/学校联考、文化课/专业课、考试状态筛选。
  - 考试状态统一为未开始、进行中、评审中、已公示。
  - 学生状态统一为无权限、待开始、可参加、已交卷、未参加、已出分。
  - 考试详情上方展示富文本考试介绍，权限说明与考试时间安排同排展示，成绩摘要按状态联动。
  - `#/exam-answer`：正式考试答题页，支持倒计时、交卷确认、大题结构、综合题子题和标记本题。
  - `#/exam-analysis`：考试成绩与解析页，展示得分/总分、题目状态、单题解析和富文本解析占位。
  - `#/exam-rank`：考试排行页，支持考生排行、学校排行、分页和每页 20/30/50 条配置。
