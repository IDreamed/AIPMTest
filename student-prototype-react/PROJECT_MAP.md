...
- 身份切换和标注按钮是原型展示控件，当前收纳在右上角 `演示控制` 浮层，不进入顶部主导航。
...
- 推荐课程试看页的开通入口文案为“申请入校”，点击打开全局申请入校弹窗。
...
- 学习中心不再提供多学校/多班级下拉切换，直接展示当前班级和所属学校。
- 班级绑定专业大类，专业课试卷权限由当前班级绑定大类决定。
- 学习中心首页只做摘要，不承接全部完整列表。
- 学习中心首页结构：
  - 当前班级信息：突出班级名称，学校作为辅助信息展示，专业大类只保留一个标签。
  - 数据概览：只保留待处理事项、累计错题；累计错题提供错题本入口。
  - 学习任务摘要。
  - 学习记录。
- 学习中心首页不再展示“本周学习”“学校班级”“专业课试卷已授权”等弱价值标注。
- 学习任务摘要按资源来源拆分：
  - 课程学习。
  - 试卷练习。
  - 班级测试。
  - 考试安排。
  - 班级答疑。
- 学习任务摘要各栏目入口按钮放在栏目右上角，作为明显的二级入口；课程学习入口文案为“查看全部课程”。
- 学习任务摘要中的卡片标签只表达状态，不重复表达资源类型，例如课程只显示“最近学习”“未开始”。
- 学习任务摘要的模块说明属于产品/开发说明，放入 `PrototypeNote`，默认不作为页面正文展示。
- 课程学习摘要只展示最近学习或新分配的班级课程，不混入课堂练习。
- 班级课程 `#/class-courses` 展示当前班级分配课程列表，点击进入课程学习详情；列表包含分页控件。
- 试卷练习 `#/paper-practice` 是学习中心下的独立列表页，只展示学生从试卷中心开始过/完成过的试卷，不展示未开始试卷，也不混入课程内练习。
- 试卷练习筛选维度与试卷中心保持一致：
  - 类型：文化课 / 专业课。
  - 专业或科目。
  - 来源：全部 / 官方 / 本校。
...
- 试卷练习列表字段：
  ...
  - 状态。
  - 操作。
- 试卷练习列表上方展示进行中、已完成、累计用时、筛选结果。
- 试卷练习列表点击试卷名称进入继续练习或解析。
- 试卷练习列表包含分页控件，当前每页条数可选 5、10、20。
- 试卷练习答题与解析复用试卷中心已有页面：
...
- 我的考试 `#/my-exams` 是学习中心下的个人正式考试列表：
  ...
  - 班级测试不进入我的考试。
  - 统计只展示相关考试、已交卷、已出分；不展示待关注。
- 错题本入口放在首页“累计错题”统计卡中；学习记录区域不再重复展示错题巩固卡片。
- 学习记录是个人学习数据，高于班级，但当前阶段不提供多学校/多班级筛选。
...
- 运营后台入口在右上角 `演示控制` 浮层中，点击“进入运营后台”进入 `#/admin`。
- 运营后台不套用学生端导航和身份壳层。
- 平台资讯管理 `#/admin/news`：
  - 资讯不提供封面。
  - 资讯不提供排序，按发布时间倒序展示。
...
- 课时状态只使用：未开始、进行中、已完成。
- 试卷状态只使用：未开始、进行中、已完成。
- 考试状态独立于课程/课时/试卷状态，可使用：未开始、进行中、评审中、已公示、已结束。
- 班级测试学生状态独立，可使用：未开始、缺考、已交卷、已出分。
...
- 改顶部栏、账号入口、演示控制、通用组件：`src/components/ui.jsx`
- 改路由：`src/App.jsx`
